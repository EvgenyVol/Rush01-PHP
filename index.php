...
if ($state='game' && $_SESSION && isset($_SESSION['user'])) {
    ...
    if (count($accounts) % 2 == 0) {
        $state = 'game';
        //start game here
        $palyer1 = new Player(['name' => $_SERVER['players'][$numberOfPlayers - 2], 'position' => 1]);
        $palyer2 = new Player(['name' => $_SERVER['players'][$numberOfPlayers - 1], 'position' => 2]);
        $_SESSION['games'][] = new Game($player1, $player2);
    } else {
        $state = 'menu';
    }
}
